...
final TextEditingController _searchController = TextEditingController();
  ...
  child: FutureBuilder<QuerySnapshot>(
    ...
    builder: (context, snapshot) {
      ...
      for (var tickerDoc in tickerDocs) {
        stocks.add(Stock(tickerDoc['ticker'], tickerDoc['amount'], tickerDoc['bought'], tickerDoc['term'], nowDate));
  ...
  schedule2.forEach((key, value) { 
    var reca = Recomendation(key, value['count'], value['date'].toString());
    recos.add(reca);
  });
  ...
  itemBuilder: (context, index) {
    ...
    final amountOrStocksPerPeriod = recommendation.count.toString();
    /*final nextDate = recommendation.length > 2 ? 
      DateTime.parse(recommendation[2]) 
      : null;*/
    final nextDate = DateTime.parse(recommendation.investDay);
...
Map AVGstrategy(List<Stock> portfolio, num period_budget, num period) {
    ...
    while (stabilization == false) {
      ...
      for (var stockInfo in sortedStock) {
          ...
          stockInfo[1]['boughtPercent'] = (100 * ((stock.bought + count) / stock.amount)).round();
          ...
          if (count != 0 && schedule[stock.ticker]['date'] == 'none') {
              schedule[stock.ticker]['count'] = count;
              var lastInvestDay = DateTime.parse(stock.lastInvestDay).add((Duration(days: period.toInt()*n)));
              schedule[stock.ticker]['date'] = lastInvestDay;
          }
